...
│  ├─ motion.js              แอนิเมชันตามการเลื่อน + เปลี่ยนพื้นหลังตามฉาก + ปุ่มล่างบนมือถือ
...
## แอนิเมชันตามการเลื่อน (v3)

- **เนื้อหาเห็นครบเสมอ** แม้ JavaScript ไม่ทำงาน สถานะเริ่มต้นที่ซ่อนอยู่ทั้งหมดผูกกับ `html.anim` ซึ่ง `motion.js` ใส่ให้เมื่อเริ่มทำงานแล้วเท่านั้น และถอดออกถ้าเกิด error
- `i18n.js`, `motion.js`, `main.js` เขียนแบบ ES2017 (ไม่ใช้ `?.` `??` `??=`) เพราะ in-app browser รุ่นเก่า (LINE, Facebook, Android WebView) จะปฏิเสธทั้งไฟล์ถ้าเจอ syntax ใหม่
- Hero: ปักค้าง (`html.pin`) เมื่อจอสูง ≥ 520px และไม่ได้ตั้ง reduced motion — `--t` 0→1 ขับหัวข้อลอยหาย มือถือซูมขึ้น อาหารบินเข้าเครื่อง ตัวเลขงบคงเหลือนับลด ถ้าจอเตี้ย (มือถือแนวนอน) แสดงแบบนิ่งพร้อมผลลัพธ์สุดท้าย
- ส่วนอื่น: `data-reveal="up|left|right|zoom|flip|flip-left|flip-right|pop"` เล่นเมื่อเลื่อนมาถึง, `data-progress` ได้ค่า `--p` ตามการเลื่อน (แถบหน่วยไทย, ข้อความซูมออก, timeline)
- พื้นหลังทั้งหน้าคือ `.scene-bg` เปลี่ยนสีตาม `data-bg` ของส่วนที่อยู่กลางจอ (`base`, `mint`, `sky`, `peach`, `lime`, `deep`)
- มือถือ: ปุ่ม "เริ่มใช้ฟรี" ติดล่างจอหลังผ่าน hero ซ่อนเมื่อถึงวิธีติดตั้งหรือฟอร์มติดต่อ
- ทดสอบแล้ว: 320×568, 360×740, 390×844, 740×360, 768×1024, 1024×768, 1366×820 ไม่มี horizontal scroll
